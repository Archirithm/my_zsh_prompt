...
std::string env_color(const char *name, const char *fallback) {
        const char *value = std::getenv(name);
        if (value != nullptr && is_hex_color(value))
                return value;
        return fallback;
}
...
PromptPalette prompt_palette() {
        return {
                {env_color("CLAVIS_PROMPT_PATH_BG", "#89B4FA"),
                        env_color("CLAVIS_PROMPT_PATH_FG", "#1E1E2E")},
                {env_color("CLAVIS_PROMPT_GIT_BG", "#CBA6F7"),
                        env_color("CLAVIS_PROMPT_GIT_FG", "#1E1E2E")},
                {env_color("CLAVIS_PROMPT_LANG_BG", "#A6E3A1"),
                        env_color("CLAVIS_PROMPT_LANG_FG", "#1E1E2E")},
                {env_color("CLAVIS_PROMPT_TIME_BG", "#F9E2AF"),
                        env_color("CLAVIS_PROMPT_TIME_FG", "#1E1E2E")},
                {env_color("CLAVIS_PROMPT_ERROR_BG", "#F38BA8"),
                        env_color("CLAVIS_PROMPT_ERROR_FG", "#1E1E2E")},
                env_color("CLAVIS_PROMPT_CONNECTOR", "#A6ADC8"),
                env_color("CLAVIS_PROMPT_ARROW", "#A6E3A1"),
        };
}
...
RenderedPill render_pill(const std::string &background,
        ...
        return render_pill(background, foreground, inner_text, inner_text);
}

RenderedPill render_pill(const std::string &background,
                const std::string &foreground, const std::string &inner_zsh_code,
                const std::string &visible_text) {
        std::string zsh_code = "%F{" + background + "}%K{" + background +
                "}%F{" + foreground + "}" + inner_zsh_code + "%k%F{" +
                background + "}%f";
...
RenderedPill get_lang_env() {
        ...
        return render_pill(
                palette.language.background, palette.language.foreground, inner_text);
}
...
RenderedPill get_directory_pill() {
        ...
        const PromptPalette palette = prompt_palette();
        return render_pill(palette.path.background, palette.path.foreground, inner_text);
}
...
RenderedPill get_git_pill() {
        ...
        const PromptPalette palette = prompt_palette();
        return render_pill(palette.git.background, palette.git.foreground, inner_text);
}

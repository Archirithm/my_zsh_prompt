...
void test_prompt_palette() {
        setenv("CLAVIS_PROMPT_PATH_BG", "#123456", 1);
        setenv("CLAVIS_PROMPT_PATH_FG", "#abcdef", 1);
        setenv("CLAVIS_PROMPT_CONNECTOR", "not-a-color", 1);

        PromptPalette palette = prompt_palette();
        check(palette.path.background == "#123456", "environment path background");
        check(palette.path.foreground == "#abcdef", "environment path foreground");
        check(palette.connector == "#A6ADC8", "invalid color fallback");

        RenderedPill pill = render_pill(
                palette.path.background, palette.path.foreground, "test");
        check(contains(pill.zsh_code, "%K{#123456}"), "pill background");
        check(contains(pill.zsh_code, "%F{#abcdef}"), "pill foreground");
        check(pill.visible_width == visible_width("test"),
                "palette does not change visible width");
}
